// app.js — Updated for new recommend.js API
// IDs used: searchForm, query, results, cards, status
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const getSeen = () => {
    try {
      return JSON.parse(localStorage.getItem("seenCigars") || "[]");
    } catch {
      return [];
    }
  };
  ...
  // Show status & disable button when busy
  ...
  form?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const res = await fetch(API_URL, {
        ...
        body: JSON.stringify({ cigar: q, avoid })
      });
      ...
      // Fire analytics if Plausible exists
      try {
